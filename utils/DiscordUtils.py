...
async def updateInfo(bot: commands.Bot):
    try:
        ...
        oneEmoji = bot.get_emoji(id=int(guildConfig['number_one_id']))
        twoEmoji = bot.get_emoji(id=int(guildConfig['number_two_id']))
        threeEmoji = bot.get_emoji(id=int(guildConfig['number_three_id']))
        fourEmoji = bot.get_emoji(id=int(guildConfig['number_four_id']))
        ...
        infoEmbed1.add_field(name="**:small_blue_diamond: Our favorite music!**", inline=False, value="Everyone here has their unique tastes: grunge to funk, pop to indie, and everything in between.\n\n" +
            f"{oneEmoji} <#{guildConfig['main_st_channel_id']}> is your go-to place for general music discussion. This is your place to start!\n" +
            f"{twoEmoji} <#{guildConfig['playlists_channel_id']}> is where you can share your ~~fire mixtape~~ playlists with us! Check out the playlists already there too; you might find something you like.\n" +
            f"{threeEmoji} <#{guildConfig['your_favs_channel_id']}> is for posting a new song you love, or if want to share an old, over-repeated favorite!\n" +
            f"{fourEmoji} <#{guildConfig['music_irl_channel_id']}> is here for you to flex your concert experiences or your crippling vinyl addiction! Real-life photos, videos, and all real-world music go here!\n")
        infoEmbed1.add_field(name="**:small_blue_diamond: Making more music!**", inline=False, value="We're happy to have some producers and of course, music fans, in our community. They'd love to help you master your favorite DAWs!\n\n" +
            f"{oneEmoji} <#{guildConfig['show_off_channel_id']}> is for if you made something you want to share with us!\n" +
            f"{twoEmoji} <#{guildConfig['ask_the_roll_channel_id']}> go here if you want some help putting the finishing touches on your song, or need other support with producing! This is your assistance hub.\n" +
            f"{threeEmoji} <#{guildConfig['collab_channel_id']}> is your go-to when looking to collab with a fellow roll. When you're done, show us in the <#{guildConfig['show_off_channel_id']}> channel!\n" +
            f"{fourEmoji} <#{guildConfig['equipment_channel_id']}> is here when you need equipment and hardware advice, or just want to show us that new MIDI keyboard you got! One stop here will do the trick.\n")
        infoEmbed1.add_field(name="**:small_blue_diamond: Your awesome speaker and headphone setups!**", inline=False, value="From home theaters to the best headset you've ever had, you're sure to find the audiophiles among us.\n\n" +
            f"{oneEmoji} <#{guildConfig['hifi_channel_id']}> conversations can be found here! Finish setting up a new amp? Surround setup? Show us it all!\n" +
            f"{twoEmoji} <#{guildConfig['tech_recommendations_channel_id']}> can be used for some recommendations for new equipment! Wanna promote (#NotASponsor) an awesome new product? This channel has you covered.\n" +
            f"{threeEmoji} <#{guildConfig['speaker_setup_channel_id']}> for all your help untangling the speaker wires. Not sure where to put the subwoofer? Ask here!")
